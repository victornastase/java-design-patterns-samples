...
public class EmployeeCaretaker {
    ...
    public void addMemento(int empId, String mementoMessage, EmployeeMemento memento) {
        if(mementoMessage != null && mementoMessage.trim().length() != 0 && memento != null) {
            Map<String, EmployeeMemento> mementoMsgMap = mementoHistory.get(empId);
            if(mementoMsgMap == null) {
                mementoMsgMap = new HashMap<String, EmployeeMemento>();
                mementoHistory.put(empId, mementoMsgMap);
            }
            mementoMsgMap.put(mementoMessage, memento);
